...
export class StuffService {
  ...
  getStuffByType(type:string){
    this.http.get('http://localhost:3000/api/stuff/type/' + type).subscribe(
      (stuff: Thing[]) => {
        if (stuff) {
          this.stuff = stuff;
          this.emitStuff();
...
getThingbystar():Observable<any>{
  return this.http.get<any>('http://localhost:3000/api/review/star' );
}
  ...
  deleteThing(id: string) {
     ...
     return this.http.delete('http://localhost:3000/api/stuff/' + id);
  }
}
